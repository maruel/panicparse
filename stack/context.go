...
func getPrefixAndGoroutine(trimmed []byte) ([]byte, *Goroutine) {
	match := reRoutineHeader.FindSubmatch(trimmed)
	if match == nil {
		return nil, nil
	}
	id, ok := atou(match[2])
...
func (s *scanningState) scan(line []byte) (bool, error) {
	...
	case betweenRoutine:
		...
		if prefix, g := getPrefixAndGoroutine(trimmed); g != nil {
			g.First = len(s.Goroutines) == 0
