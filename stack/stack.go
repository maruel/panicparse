...
type Signature struct {
	...
	// Use git grep 'gopark(|unlock)\(' to find them all.
	State       string
		...
		if goroutine == nil {
			if match := reRoutineHeader.FindStringSubmatch(line); match != nil {
				if id, err := strconv.Atoi(match[1]); err == nil {
					...
					sleep := 0
					if match2 := reMinutes.FindStringSubmatch(match[2]); match2 != nil {
						sleep, _ = strconv.Atoi(match2[2])
						match[2] = match2[1]
					}
					goroutines = append(goroutines, Goroutine{
						Signature: Signature{State: match[2], Sleep: sleep, Stack: []Call{}},
						ID:        id,
